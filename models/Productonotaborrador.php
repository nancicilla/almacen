...
class Productonotaborrador extends CActiveRecord {
     ...
     * @throws CrugeException
     */
    public function registrarProductoNotaborrador($arrayParametros)
    {
        $idnotaborrador = $arrayParametros['idnota'];
        $productonotaborrador = $arrayParametros['productonotaborrador'];
        ...
        if ($idnotaborrador != Null && isset($modelo) && isset($productonotaborrador)) {
            ...
            {
                $glosaanterior = $productonotaborrador[$i]['glosa']==null? ' - ':' - '.$productonotaborrador[$i]['glosa'].' - ';
                $glosaParcial = $causa . ', ' . $tipo.$glosaanterior . 'NOTA Nº ' . $numero;

                if ($idtipo == Tipo::model()->INGRESO) {
                    $modelo->glosa = 'INGRESO POR ' . $glosaParcial;
                    $modelo->ingreso = $productonotaborrador[$i]['cantidad'];
                    $modelo->salida = 0;
                } else {
                    $modelo->glosa = 'SALIDA POR ' . $glosaParcial;
                    $modelo->salida = $productonotaborrador[$i]['cantidad'];
                    $modelo->ingreso = 0;
                }
                $cantidadInsumo = $productonotaborrador[$i]['cantidad'];

                if($postNota['costovariable'])
                    if($productonotaborrador[$i]['costo'] > 0 && $productonotaborrador[$i]['costo'] != $productonotaborrador[$i]['costoHidden'])
                    {
                        $modelo->costo = $productonotaborrador[$i]['costo'];
                        $modelo->importefijo = true;                          
                    }

                if ($productonotaborrador[$i]['idproducto'] != Null) {
                    $modelo->idproducto = $productonotaborrador[$i]['idproducto'];
                    ...
                    if($postNota['costovariable'])
                        $SalidaIngresoImporte = $modelo->costo * $productonotaborrador[$i]['cantidad'];
            ...
            echo 'Error al registrar productos en la nota borrador.';
        }
    }
