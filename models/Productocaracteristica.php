 ...
 * Creacion: 17/03/2015
 *
 * Ultima Actualizacion: $Date: 2016-09-16 17:56:56 -0400 (vie 16 de sep de 2016) $:
 * 
...
//                                array('archivo', 'file', 'types'=>'jpg, gif, png'),
    ...
    public function registrarImagen($idProducto, $imagen, $directorioTemporal) {
        $modelCaracteristica = Caracteristica::model()->findAll('idgenero = '.Genero::model()->GENEROARCHIVO);
        for($j = 0; $j < count($modelCaracteristica); $j++)
        {
            Productocaracteristica::model()
                ->deleteAll('idproducto = '.$idProducto.' and idcaracteristica = '.$modelCaracteristica[$j]['id']);
        }
