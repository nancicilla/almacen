<?php
/*
 * Productoproducto.php
 *
 * Version 0.$Rev: 475 $
 *
 * Creacion: 17/03/2015
 *
 * Ultima Actualizacion: $Date: 2016-04-11 03:05:26 -0400 (lun 11 de abr de 2016) $:
 * 
...
//			array('idproducto, idcomplementario', 'numerical', 'integerOnly'=>true),
     ...
     */
    public function registrarComplementario($id, $complementario) {        
        $modelo = new Productoproducto();
        if ($id != Null && isset($complementario)) {
            $modelo->attributes = $complementario;
            $modelo->idproducto = $id;
            $cantidad = count($complementario);
            if($cantidad > 0)
            {
                for ($i = 1; $i <= $cantidad; $i++) {
                    foreach ($complementario[$i] as $atributo => $dato) {
                        if ($atributo == 'idcomplementario') {
                            if ($dato != Null) {
                                $modelo->idcomplementario = $dato;
                                if ($modelo->save()) {
                                    $modelo = new Productoproducto;
                                    $modelo->attributes = $complementario;
                                    $modelo->idproducto = $id;
            ...
            echo 'Error al registrar productos suplementarios.';
        }       
