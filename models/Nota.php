 ...
 * Creacion: 17/03/2015
 *
 * Ultima Actualizacion: $Date: 2019-10-08 16:14:08 -0400 (mar, 08 oct 2019) $:
 * 
...
class Nota extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('idorigen,idalmacen,idtipodocumento,idcausa', 'required', 'on' => array('insert', 'update'), 'except' => 'correccionsistema'),
            ...
            array('glosa, eliminado,descripcion,idchofer,cantidadcaja,idcausa', 'safe'),
            ...
            array('idalmacen,norden,notaTipo,id, numero, glosa, fecha, eliminado,fechaInicio,fechaFin,'
                ...
                . 'idestado, descripcion, fechaentrega, idcausa',
                'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'notaTipo' => 'Movimiento',
            'idalmacenorigen'=>'Almacén de Origen',
            'idalmacendestino'=>'Almacén de Destino',
        );
    }
    ...
    public function search() {
        ...
        if ($this->validate()) {
            ...
            if ($this->idcausa != Null) {
                $modelCausa = Causa::model()->find('id = '.$this->idcausa);
                $criteria->compare('t.idcontracuenta', $modelCausa->idcuenta);
            }
...
//            'pagination' => false,
    ...
    /*
     * Muestra las causa de cada nota
     */
    public function getMostrarCausa()
    {
        if($this->idcontracuenta != null)
        {
            $modelCausa = Causa::model()->find('idcuenta = '.$this->idcontracuenta);
            if($modelCausa == null)
                return "";
            else
                return $modelCausa->nombre;
        }
        else
            return "";
    }
     ...
     * @return array
     */
    public function validarTraspasoEntreAlmacenes($arrayParametros){
     ...
     * @return array
     */
    public function registroSalidaTraspasoEntreAlmacenes($arrayParametros){
        $idalmacenorigen = $arrayParametros['idalmacenorigen'];
        $idalmacendestino = $arrayParametros['idalmacendestino'];
        $productosorigen = $arrayParametros['productosorigen'];
        $productosdestino = $arrayParametros['productosdestino'];
...
public function registrarSolicitudDevolucionIngreso($dataService) {
        ...
        switch ($nota['idtipodocumento']) {
            case Tipodocumento::SOLICITUDDEVOLUCION:
                $glosa='INGRESO POR SOLICITUD DEVOLUCION Nº ' . $nota['numero'];
                $idtipodocumento= $nota['idtipodocumento'];
                break;
            case Tipodocumento::CONSIGNACION:
                $glosa='INGRESO POR DEVOLUCION CONSIGNACION Nº ' . $nota['numero'];
                $idtipodocumento = Tipodocumento::model()->NOTAVENTACONSIGNACION;
                break;
            case Tipodocumento::DEVOLUCION_TPV:
                //"DEVOLUCION POR SOLICITUD N° 38 - AG. ARENALES"
                $glosa='INGRESO POR DEVOLUCION POR SOLICITUD Nº ' . $nota['numero'].' - '.$nota['agencia'];
                $idtipodocumento = $nota['idtipodocumento'];
                break;
        }
        $model = new Nota();
    ...
    public function registrarSolicitudSalidaBaja($dataService) {
        ...
        switch ($nota['idtipodocumento']) {
            case Tipodocumento::SOLICITUDDEVOLUCION:
                $glosa='SALIDA BAJA POR SOLICITUD DEVOLUCION Nº ' . $nota['numero'];
                $idtipodocumento= $nota['idtipodocumento'];
                break;
            case Tipodocumento::CONSIGNACION:
                $glosa='SALIDA BAJA POR DEVOLUCION CONSIGNACION Nº ' . $nota['numero'];
                $idtipodocumento = Tipodocumento::model()->NOTAVENTACONSIGNACION;
                break;
            case Tipodocumento::DEVOLUCION_TPV:
                //"DEVOLUCION POR SOLICITUD N° 38 - AG. ARENALES"
                $glosa='SALIDA BAJA POR DEVOLUCION POR SOLICITUD Nº ' . $nota['numero'].' - '.$nota['agencia'];
                $idtipodocumento = $nota['idtipodocumento'];
                break;
        }
        $model = new Nota();
