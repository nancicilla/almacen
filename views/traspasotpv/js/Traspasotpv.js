...
//        if(grid.row(f).get('permitirdecimal') == 0) {
//            if (!(cantidad % 1 == 0)) {
//                grid.row(f).attributes('cantidadenviada', {validate: false, tooltip: 'La cantidad de éste producto, NO debe contener decimales'});
//                verificaEntero++;
//            } else {
//                grid.row(f).attributes('cantidadenviada', {validate: true, tooltip: ''});
//            }
//        }
    }
//    if(verificaEntero>0){
////        bootbox.alert('Existen cantidades no validas');
//        Traspasotpv.showMessageError('La cantidad no debe contener Decimales');
//        error = true;
//    }
    if (cantidad==0){
        error = false;
    ...
    for (var f = 1; f <= grid.rows; f++)
    {
        ...
        if (this.action === 'recepcionSolicitud' || this.action === 'view'){
            if (saldoDisponible<0){
                saldoDisponible=saldoDisponible*-1;
            }
            Disponiblereal = saldoDisponible;
        }
        if (this.action === 'view'){
            ...
            jQuery.ajax({
                ...
                success: function (resultado) {
                    if (resultado == 0) {
                        ...
                        grid.rowSelected().set('permitirdecimal', null);
                        ...
                        grid.rowSelected().set('precio', arrayDatosProducto.precio);
                        grid.rowSelected().set('permitirdecimal', arrayDatosProducto.permitirdecimal);
                    }
                    return;
                },
...
Traspasotpv.eliminarProducto = function () {
    ...
    if ((ultimoestado == 4) && verifica == 1 && grid.rowSelected().get('cantidadsolicitada')==0) {
        var fild = grid.rowSelected();
        fild.delRow();
        return;
    }

    if ((ultimoestado == 27) && verifica == 1 && grid.rowSelected().get('cantidadsolicitada')==0) {
        var fild = grid.rowSelected();
        fild.delRow();
        return;
    }
    if (ultimoestado == 27 && verifica == 1 && grid.rowSelected().get('cantidadsolicitada')>0) {
        bootbox.alert("No puedes eliminar este item");
        return;
    }
...
//        if(grid.row(f).get('permitirdecimal') == 0) {
//            if (!(cantidad % 1 == 0)) {
//                grid.row(f).attributes('cantidadsolicitada', {validate: false, tooltip: 'La cantidad de éste producto, NO debe contener decimales'});
//                verificaEntero++;
//            } else {
//                grid.row(f).attributes('cantidadsolicitada', {validate: true, tooltip: ''});
//            }
//        }
    }
//    if(verificaEntero>0){
...
//        if(grid.row(f).get('permitirdecimal') == 0) {
//            if (!(cantidad % 1 == 0)) {
//                grid.row(f).attributes('cantidad', {validate: false, tooltip: 'La cantidad de éste producto, NO debe contener decimales'});
//                verificaEntero++;
//            } else {
