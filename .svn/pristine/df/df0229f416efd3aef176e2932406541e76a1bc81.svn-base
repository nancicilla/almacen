...
TraspasoEntreAlmacenes.init = function () {
    try {
        ...
        if (this.action == 'traspasoEntreAlmacenes') {
            ...
            this.gridEventClickRow('ProductosTraspasoOrigen', 'TraspasoEntreAlmacenes.showDisponible();');
            this.gridEventBlur('ProductosTraspasoOrigen', 'TraspasoEntreAlmacenes.hiddenDisponible();');
        }
    } catch (err) {
...
TraspasoEntreAlmacenes.traspasoPedidoEspecial = function(options) {
//    var iddocumentoproducto = (options['iddocumentoproducto'] != null) ? options['iddocumentoproducto'] : '';
//    alert(iddocumentoproducto);
//    return;
    this.action = 'traspasoEntreAlmacenes';
    var options = this.getOptions(options);
//    options.varSend = 'idPedido=' + idKey;
...
TraspasoEntreAlmacenes.showDisponible = function () {
    ...
    if (grid.rowSelected().get('idproducto') != '') {
        var disponibleTotal = TraspasoEntreAlmacenes.cantidadTotalProductoNota(grid.rowSelected().get('idproducto'));
...
TraspasoEntreAlmacenes.hiddenDisponible = function () {
    var divSaldo = this.ById('divSaldo');
    divSaldo.parentElement.style.visibility = 'hidden';
}
