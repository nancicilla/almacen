...
Producto.init = function () {
    try {
        ...
        if (this.action === 'reportestockminimo'){
            this.nodespliega(this.Id('divProveedor'));
            this.nodespliega(this.Id('divFamilia'));
            this.nodespliega(this.Id('divProducto'));
            $('#' + this.Id('grupoproducto')).change(function() {
            if ($('#' + Producto.Id('grupoproducto')).val()==1){
                $('#' + Producto.Id('idfamilia')).val('');
                $('#' + Producto.Id('familia')).val('');
                $('#' + Producto.Id('idproductoindividual')).val('');
                $('#' + Producto.Id('producto')).val('');
                $('#' + Producto.Id('idproveedor')).val('');
                $('#' + Producto.Id('proveedor')).val('');
            }
            if ($('#' + Producto.Id('grupoproducto')).val()==2){
                Producto.sidespliega(Producto.Id('divProveedor'));
                $('#' + Producto.Id('idfamilia')).val('');
                $('#' + Producto.Id('familia')).val('');
                $('#' + Producto.Id('idproductoindividual')).val('');
                $('#' + Producto.Id('producto')).val('');
            }else{
                Producto.nodespliega(Producto.Id('divProveedor'));
            }
            if ($('#' + Producto.Id('grupoproducto')).val()==3){
                Producto.sidespliega(Producto.Id('divFamilia'));
                $('#' + Producto.Id('idproveedor')).val('');
                $('#' + Producto.Id('proveedor')).val('');
                $('#' + Producto.Id('idproductoindividual')).val('');
                $('#' + Producto.Id('producto')).val('');
            }else{
                Producto.nodespliega(Producto.Id('divFamilia'));
            }
            if ($('#' + Producto.Id('grupoproducto')).val()==4){
                Producto.sidespliega(Producto.Id('divProducto'));
                $('#' + Producto.Id('idproveedor')).val('');
                $('#' + Producto.Id('proveedor')).val('');
                $('#' + Producto.Id('idfamilia')).val('');
                $('#' + Producto.Id('familia')).val('');
            }else{
                Producto.nodespliega(Producto.Id('divProducto'));
            }
            
        });
            $("#" + this.Id("almacen")).change(function () {
                $("#" + Producto.Id("producto")).autocomplete({source: 'almacen/producto/autocompleteProductoStock?idalmacen=' + this.value});
                $("#" + Producto.Id("producto")).val('');
                $("#" + Producto.Id("idproductoindividual")).val('');
            });
        }
        ...
        $('#btnBuscarProductos').click(function(){
            THIS.cargarProductoStock();
        });
        $('#btnBuscarProductosSolicitud').click(function(){
            THIS.cargarProductoStockSolicitud();
        });
    } catch (err) {
...
Producto.options = function () {   
    ...
    this.setActions('reportestockminimo', {
        layerHeight: 356,
        WindowWidth: 1270,
        WindowHeight: 575,
        WindowTitle: 'Stock Minimo',
        initButtons: 'salir,asignar,cancel,print',
        layerEndOn: false,
        ableBackWindow: true
    });
    this.setActions('solicitudtockminimo', {
        layerHeight: 356,
        WindowWidth: 1270,
        WindowHeight: 575,
        WindowTitle: 'Generar Solicitud',
        initButtons: 'salir,generar,cancel',
        layerEndOn: false,
        ableBackWindow: true
    });
...
Producto.print=function(){   
    if (this.action === 'reporteConsumoPromedioProductos') {
        ...
        if (this.action === 'reportestockminimo') {
        var datos = this.prepareSend($('#' + this.groupForm).serialize());
        var urlCompleta = 'reportestockminimoPDF?' + datos;
        this.openUrl(urlCompleta);
    }
}
...
Producto.reportestockminimo = function (options) {
    this.action = 'reportestockminimo';
    this.open(this.getOptions(options));

};
Producto.beforeAsignarstockminimo = function () {
    var error = false;//false es no existe error antes de crear formulario
    var grid = this.getSGridView('ProductoStock');
    return error;
};
Producto.asignarstockminimo = function (option) {
    var error= false;
    var idalmacen = Producto.get('almacen');
    var idfamilia = Producto.get('idfamilia');
    var idproductoindividual = Producto.get('idproductoindividual');
    var grupoproducto = Producto.get('grupoproducto');
    var fechaFin = Producto.get('fechaFin');
    var fechaInicio = Producto.get('fechaInicio');
    var tipomovimiento = Producto.get('tipomovimiento');
    var idproveedor = Producto.get('idproveedor');
    
    var url = this.urlIni+this.url+'/AsignarStockMinimo';
    var THIS = this;
    var data = 'idalmacen=' + idalmacen + '&fechaInicio=' + fechaInicio + '&fechaFin=' + fechaFin + '&tipomovimiento=' + tipomovimiento + '&idproveedor=' + idproveedor + '&idfamilia=' + idfamilia + '&idproductoindividual=' + idproductoindividual + '&grupoproducto=' + grupoproducto +'&groupForm=' + this.groupForm;
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (myJsonString.error == 0) {
                THIS.showMessage(myJsonString.message);
                error = false;
            }
            if (myJsonString.error == 1) {
                console.log(myJsonString.productos);
                var productos = myJsonString.productos;
                var cadena = "";
                for(i=0;i<productos.length;i++){
                    cadena=cadena+"(" +myJsonString.productos[i]['codigo']+") "+ myJsonString.productos[i]['nombre'] + "<br>";
                }
                THIS.showMessageError("Existen productos con más de un proveedor: <br>"+cadena);
                error =true;
            }
            if (myJsonString.error == 2) {
                THIS.showMessageError(myJsonString.message);
                error = true;
            }
            if (myJsonString.error == 3) {
                THIS.showMessageError(myJsonString.message);
                error = true;
            }
        },
...
Producto.cargarProductoStock = function() {
    var fechaInicio = $('#' + this.Id('fechaInicio')).val();
    var fechaFin = $('#' + this.Id('fechaFin')).val();
    var idalmacen = $('#' + this.Id('almacen')).val();
    var tipomovimiento=$('#' + this.Id('tipomovimiento')).val();
    var grupoproducto=$('#' + this.Id('grupoproducto')).val();
    var idproveedor=$('#' + this.Id('idproveedor')).val();
    var idfamilia=$('#' + this.Id('idfamilia')).val();
    var idproductoindividual=$('#' + this.Id('idproductoindividual')).val();

    var url = this.urlIni+this.url+'/loadProductoStock';
    var THIS = this;
    var data = 'idalmacen=' + idalmacen + '&fechaInicio=' + fechaInicio +
               '&fechaFin=' + fechaFin + '&tipomovimiento=' + tipomovimiento + '&grupoproducto=' + grupoproducto +
               '&idproveedor=' + idproveedor + '&idfamilia=' + idfamilia + '&idproductoindividual=' + idproductoindividual +
               this.gestionSchemaMain() + '&groupForm=' + this.groupForm;
    var container = 'divProductoStock';
    ...
    jQuery.ajax({
            ...
            success: function (data) {
                ...
                var grid = THIS.getSGridView('ProductoStock');
                if (grid.rows>0){
                    if (grid.row(1).get('id')>0){
                        grid.row(1).ById('codigo').click();
                    }
                }
            },
            error: function (jqXHR, status) {  
...
Producto.solicitudtockminimo = function (options) {
    this.action = 'solicitudtockminimo';
    this.open(this.getOptions(options));

};
Producto.cargarProductoStockSolicitud = function() {
    var fechaInicio = $('#' + this.Id('fechaInicio')).val();
    var fechaFin = $('#' + this.Id('fechaFin')).val();
    var idalmacen = $('#' + this.Id('almacen')).val();
    
    var url = this.urlIni+this.url+'/loadProductoStockSolicitud';
    var THIS = this;
    var data = 'idalmacen=' + idalmacen + '&fechaInicio=' + fechaInicio +
               '&fechaFin=' + fechaFin +                
               this.gestionSchemaMain() + '&groupForm=' + this.groupForm;
    var container = 'divProductoStocksolicitud';
...
Producto.verificarGrid = function () {
    var grid = this.getSGridView('ProductoStock');
    for (var f = 1; f <= grid.rows; f++)
    {
        if(grid.row(f).get('id') > 0 && grid.row(f).get('proveedor') == '') {
            grid.row(f).attributes('nombre', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('coduniversal', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('codigo', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('stockredondeado', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('stockminimo', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('stockajustado', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('saldo', {'style': {'color': 'red', 'font-weight': 'bold'}});
            grid.row(f).attributes('proveedor', {'style': {'color': 'red', 'font-weight': 'bold'}});
        }
    }
};
Producto.generarSolicitud = function (option) {
    var error= false;
    var idalmacen = Producto.get('almacen');
    var fechaFin = Producto.get('fechaFin');
    var fechaInicio = Producto.get('fechaInicio');
    
    var url = this.urlIni+this.url+'/generarSolicitud';
    var THIS = this;
    var data = 'idalmacen=' + idalmacen + '&fechaInicio=' + fechaInicio + '&fechaFin=' + fechaFin + '&groupForm=' + this.groupForm;
