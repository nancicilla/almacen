    ...
    jQuery.ajax({
       ...
       success: function (data, status, jqXHR) {  
           //&&!data.terminada.estado
            ...
            if((data.iniciada.estado ||data.enproceso.estado || data.entrega.estado || data.terminada.estado)){
            ...
            if(data.cierre.estado) {
                bootbox.alert("La orden está cerrada, no se pueden registrar entregas.");
                return;} 
...
admOrden.registrarResidual = function () {
    ...
    jQuery.ajax({
        ...
        success: function (data, status, jqXHR) {
            ...
            if (data.cierre.estado) {
                bootbox.alert("La orden está cerrada, no se pueden registrar subproductos.");
                return;
            }
            if (!data.enproceso.estado || !data.entrega.estado) {
                bootbox.alert("No se puede realizar la operación.");
...
admOrden.registrarInsumo = function () {
    gridOrdenAdmin = this;
    jQuery.ajax({
        type: "GET",
        url: "produccion/orden/VerificaEstadosOrden?idorden=" + SGridView.getSelected('id'),
        ...
        success: function (data, status, jqXHR) {
            //!data.devolucion.estado&&            
            if (!data.cierre.estado && (data.planificada.estado || data.enproceso.estado || data.iniciada.estado)) {
                //var id=SGridView.getSelected('id');
                var options = gridOrdenAdmin.getOptions();
                options.varSend = '&planificada='+data.planificada.estado+'&enproceso='+data.enproceso.estado+'&iniciada='+data.iniciada.estado;
                produccionOrden.registrarInsumo(options);
                return;
            }
            
            if (data.cierre.estado) {
                bootbox.alert("La orden está cerrada, no se pueden registrar salidas.");
                return;
            }
            
            if (!data.enproceso.estado || !data.iniciada.estado) {
                bootbox.alert("No se puede realizar la operación.");
                return;
            }
...
admOrden.seguimiento = function () {   
    produccionOrden.seguimiento(this.getOptions());
    return;
}